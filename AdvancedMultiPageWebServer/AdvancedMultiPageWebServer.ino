...
IPAddress timeServer(82, 219, 4, 31); // 0.uk.pool.ntp.org
...
PROGMEM prog_char content_page2[] = "<hr /><h3>Sensors</h3><p>\002</p><p>\002</p>";
...
PROGMEM prog_char content_page4[] = "<hr /><h3>Time</h3><p>\002</p><p>\002</p>";
...
long prevMillis_dht = 0; 
const long diff_dht = 10000; //10 seconds
long prevMillis_ntp = 0; 
const long diff_ntp = 60000; //60 seconds
long targetMillis_ntp_rply = 1000000; //set high so this does not trigger straight away, the ntp event will set this 
unsigned long secsSince1900; // ntp updated clock counter  
unsigned long targetTick_ms = 1000; // when is the next tick of the clock  
...
void loop()
{
    ...
    //use a simple compare every time round the loop
    //and do the complex calculation once a second so as not to waste cycles
    if(currentMillis > targetTick_ms) // is it time to tick the clock
    {
      targetTick_ms = currentMillis / 1000; //round to seconds
      targetTick_ms *= 1000; //scale back up to ms
      targetTick_ms += 1000; //add a second
      secsSince1900++; //tick the clock
    }
    
    //check for wrap
    if(currentMillis < prevMillis_dht)
      prevMillis_dht = currentMillis;
    if(currentMillis < prevMillis_ntp)
      prevMillis_ntp = currentMillis;
    
    //request the time every minute
    if(currentMillis - prevMillis_ntp > diff_ntp)
    {
      prevMillis_ntp = currentMillis;
      targetMillis_ntp_rply = currentMillis + 1000; //check for reply 1 second from now
      
      sendNTPpacket(timeServer); // send an NTP packet to a time server
      Serial.println("Request ntp" );
    }
    //chesck for ntp reply
    if(currentMillis > targetMillis_ntp_rply)
    {
      if ( Udp.parsePacket() )
      {  
          //dont check againt until after the next request
          targetMillis_ntp_rply = currentMillis + ( diff_ntp * 2);
        ...
        else
        {
          ...
          targetMillis_ntp_rply = currentMillis + 1000;
        }

    }
    
    //read the temperature sensor every 5 seconds
    if(currentMillis - prevMillis_dht > diff_dht)
    {
      prevMillis_dht = currentMillis;
...
void printUTCTime(unsigned long secsSince1900)
{
...
//        client.print("</b>");
        ...
        {
          case 0:  // DHT22 Temperature reading
            client.print("<b>Temperature ");
            client.print(DHT.temperature);
            client.print("</b>");
          break;
          case 1:  // DHT22 Humidity reading
            client.print("<b>Humidity ");
      ...
      case 3:  // page 4
        switch (nSubstituteIndex)
        {
          case 0:  
            // print the hour, minute and second:
            client.print("The UTC time is ");       // UTC is the time at Greenwich Meridian (GMT)
